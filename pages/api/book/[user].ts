...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
    ...
    const AIRTABLE_DB_BASE = "app6JQG5sqkzb9K7a";
    const AIRTABLE_DB_NAME = "Bookings (production)";

    if (isProduction) {
        const base = new airtable({ apiKey: AIRTABLE_API_KEY }).base(AIRTABLE_DB_BASE);
        await base(AIRTABLE_DB_NAME).create({
            ID: hashUID,
            "Provider ID": currentUser.id,
            "Provider Name": currentUser.name,
            "Provider Email": currentUser.email,
            "Provider Timezone": currentUser.timeZone,
            "Booker ID": +req.body.id ?? undefined,
            "Booker Name": req.body.name,
            "Booker Email": req.body.email,
            "Booker Timezone": "",
            Length: 30,
            Notes: req.body.notes,
            "Booking Date": evt.startTime,
            Type: selectedEventType.title,
        });
    }
